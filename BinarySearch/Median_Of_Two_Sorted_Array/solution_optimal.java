class Solution {
    public double median(int[] arr1, int[] arr2) {
        ...
        while(low <= high) {
            ...
            int mid1 = (low + high) >>> 1; // Unsigned right shift by 1 is faster than /2 (same as (low + high) / 2)
            ...
            if(l1 <= r2 && l2 <= r1) {
                // Condition for finding median
                if(n % 2 == 1) return Math.max(l1, l2); // for odd elements
                else return (Math.max(l1, l2) + Math.min(r1, r2)) / 2.0; // for even elements
            } else if(l1 > r2) {
